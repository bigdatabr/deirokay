...
class Regex(BaseStatement):
    """Check if the rows of a scoped DataFrame obey to certain regex pattern."""
    ...
    def report(self, df):
        ...
        for column in df.columns:
            matches = df[column].apply(lambda x: bool(re.fullmatch(self.pattern, str(x))))
            report[column] = {
                'matching_rows': int(matches.sum()),
                'matching_rows_%': float(100*matches.sum()/len(matches)),
                'not_matching_rows': int((~matches).sum()),
                'not_matching_rows_%': float(100.0*(~matches).sum()/len(matches)),
            }
    ...
    def result(self, report):
        for col_report in report:
            if not report[col_report].get('matching_rows_%') >= self.at_least_perc:
                return False
        return True
