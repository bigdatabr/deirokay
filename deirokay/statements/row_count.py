...
class RowCount(BaseStatement):
    ...
    - `min`: The minimum number of rows. If None, no minimum is enforced.
    Default: None.
    - `max`: The maximum number of rows. If None, no maximum is enforced.
    Default: None.
    ...
    sharply downwards (below 5% of its 7-day historical average), which
    could be a sign of failure to receive transactions from some
    branches.
    ...
    into a single line. In JSON you would have to put everything in the
    same line;
