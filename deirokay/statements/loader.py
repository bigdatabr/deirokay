...
def _load_custom_statement(location: str) -> Type[BaseStatement]:
    ...
    if "::" not in location:
        raise ValueError(
            "You should pass your class location using the"
            " following pattern:\n"
    ...
    if not issubclass(cls, BaseStatement):
        raise ImportError(
            "Your custom statement should be a subclass of" " BaseStatement"
        )

    return cls


def statement_factory(statement: DeirokayStatement, backend: Backend) -> BaseStatement:
    ...
    if stmt_type == "custom":
        ...
        if not location:
            raise KeyError("A custom statement must define a `location`" " parameter.")
