...
def prepare_history_s3(require_s3_test_bucket):
    ...
    def delete_s3_prefix(bucket, prefix):
        for obj in s3.list_objects(Bucket=bucket, Prefix=prefix).get("Contents", []):
...
@pytest.mark.parametrize("backend", list(Backend))
def test_data_validation_with_jinja_using_s3(monkeypatch, prepare_history_s3, backend):
...
def test_s3_paths_with_same_prefixes(
    require_s3_test_bucket, _create_files_with_same_prefixes
):
