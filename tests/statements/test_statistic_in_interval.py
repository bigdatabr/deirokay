...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("COD_SETVENDAS", "mode", {">=": 6001628, "<=": 6001628}, "pass"),
        ("NUMERO_PDV_ORIGIN", "min", {">=": 10.5, "<=": 11.5}, "pass"),
        ("NUMERO_PDV_ORIGIN", "max", {">=": 12, "<=": 12.5}, "pass"),
    ],
)
def test_statistic_in_interval(scope, statistic, intervals, result, backend):
    ...
    report = validate(df, against=assertions, raise_exception=False)["items"][0][
        "statements"
    ][0]["report"]
...
def test_profile(backend):
    ...
    assertions = {
        "name": "profiling",
        "items": [{"scope": "NUM_TRANSACAO01", "statements": [generated_prof]}],
    }
