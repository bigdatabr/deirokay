...
* **Simple to use**: instead of writing your data validation in pure Python, you can create your rules in a simple and intuitive *declarative language* (`see on Wikipedia <https://en.wikipedia.org/wiki/Declarative_programming>`_) with just a few lines of configuration. This leads to a more human-readable format to express your validations, making it possible for people with different levels of expertize to engage and contribute with it.
...
* **Customizable**: you may create and reuse your own statement classes to express rules tailored to your needs, instead of being limited to the standard library of Deirokay Statements.
* **Multiple sources**: you can validate data from multiple sources, including files in several extensions, relational databases or existing Pandas DataFrames.
...
Deirokay also integrates easily with `Apache Airflow <https://airflow.apache.org/docs/>`_ with its built-in :ref:`DeirokayOperator<Deirokay Airflow Operator>`.
